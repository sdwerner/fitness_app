...
class AnalyticsDataProcessor:
    ...
    def get_leaderboard_data(_self, limit: int = 50) -> pd.DataFrame:
        ...
        SELECT 
            ...
            AVG(p.points) as avg_points_per_activity
        FROM users u
        LEFT JOIN performances p ON u.id = p.user_id
        LEFT JOIN teams t ON u.team_id = t.id
        GROUP BY u.id, u.username, u.full_name, u.gender, u.age_group, u.location, t.name
    ...
    def get_team_leaderboard(_self, limit: int = 20) -> pd.DataFrame:
        ...
        SELECT 
            ...
            COALESCE(SUM(p.points), 0) as total_team_points,
            COALESCE(AVG(p.points), 0) as avg_points_per_member,
            COUNT(p.id) as total_team_activities,
            MAX(p.date) as last_team_activity
        FROM teams t
    ...
    def get_sport_leaderboard(_self, sport_name: str, limit: int = 20) -> pd.DataFrame:
        ...
        WHERE s.name = ?
        GROUP BY u.id, u.username, u.full_name, s.name, s.unit
    ...
    def get_user_progress_data(_self, user_id: int) -> Dict:
        ...
        daily_query = """
        SELECT 
            DATE(p.date) as activity_date,
            COUNT(p.id) as daily_activities,
            SUM(p.points) as daily_points,
            SUM(SUM(p.points)) OVER (ORDER BY DATE(p.date)) as cumulative_points
        FROM performances p
        WHERE p.user_id = ?
    ...
    def get_user_ranking(_self, user_id: int) -> Dict:
        ...
        user_points_query = """
        SELECT COALESCE(SUM(points), 0) as user_points
        FROM performances
        WHERE user_id = ?
        """
        ...
        with _self.get_connection() as conn:
            ...
            user_rank_row = all_points_df[all_points_df['id'] == user_id]
    ...
    def get_weekly_progress(_self, user_id: int, weeks: int = 12) -> pd.DataFrame:
        ...
        SELECT 
            ...
            COUNT(p.id) as weekly_activities,
            SUM(p.points) as weekly_points,
            AVG(p.points) as avg_points_per_activity
        FROM performances p
